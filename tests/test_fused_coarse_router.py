"""Independent calculus and CUDA checks for the experimental coarse core."""
import math
import unittest
import torch


def reference(z, beta, k, gc, state, rn, qn, u, temperature, floor, dtype):
    projected = z @ state
    key2 = k.square().sum(-1)
...
def analytical(inputs, dy, ds, floor):
    ...
    z,beta,k,gc,h,r,q,u,temp=inputs
    zz=z@h; qq=q@h; kn=k.square().sum(-1)
    ...
    dq=dn.unsqueeze(-1)*u
    dh=r.transpose(-1,-2)@dy+q.transpose(-1,-2)@dq+z.transpose(-1,-2)@dz
    dh=dh+2*dr.sum(-1)[...,None,None]*h
    return (dz@h.transpose(-1,-2),da*(2-2*beta*kn),-2*(da*beta.square()).unsqueeze(-1)*k,
...
def make_inputs(bh,n,c,kdim,vdim,device,dtype=torch.float32,scale=1.):
    ...
    return tuple(x.detach().requires_grad_() for x in (z,beta,k,gc,h,r,q,u,temp))
...
class FusedGPU(unittest.TestCase):
    def test_forward_and_nine_gradients(self):
        from hattention.fused_coarse_router import fused_coarse_core
            ...
            for output_dtype in (torch.float32,torch.bfloat16):
                for scale in (1.,1e-8,0.):
                    with self.subTest(shape=shape,dtype=output_dtype,scale=scale):
                        ...
                        fy,fs,flag=fused_coarse_core(*inputs,mass,1e-6,output_dtype)
    ...
    def test_dynamic_batch_forward_and_backward(self):
        from hattention.fused_coarse_router import fused_coarse_core
        compiled = torch.compile(fused_coarse_core, fullgraph=True, dynamic=True)
    ...
    def test_diagnostic_masks(self):
        ...
        _,_,flag=fused_coarse_core(*inputs,mass,1e-6,torch.float32)
        _,_,bad,nonfinite=fused_coarse_core(*inputs,mass,1e-6,torch.float32,return_masks=True)
