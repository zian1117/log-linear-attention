...
def _project_forward(P, R, Q, H, U, Y, Z2, NUM, H2, QREAD,
                     ...
                     VT: tl.constexpr, RAW_HALF: tl.constexpr=0):
    ...
    for block in range(tl.cdiv(K, BK)):
        ...
        r = tl.load(R + batch*C*K + c[:, None]*K + key[None, :] + raw_delta*(C*K),
                    (c[:, None] < C) & (key[None, :] < K), 0)
        ar = tl.dot(r, h, ar, input_precision='tf32x3')
        q = tl.load(Q + batch*C*K + c[:, None]*K + key[None, :] + raw_delta*(C*K),
                    (c[:, None] < C) & (key[None, :] < K), 0)
        aq = tl.dot(q, h, aq, input_precision='tf32x3')
        h2 += tl.sum(tl.sum(h*h, 0), 0)
    u = tl.load(U + batch*C*V + c[:, None]*V + vv[None, :] + raw_delta*(C*V),
...
def _query_backward(DQ, DY, H, KP, DKCOEF, QG, RG, KG,
                    C: tl.constexpr, K: tl.constexpr, V: tl.constexpr,
                    BC: tl.constexpr, BK: tl.constexpr, BV: tl.constexpr, RAW_HALF: tl.constexpr=0):
    ...
    key = tl.load(KP+off+raw_delta*(C*K), mask, 0)
...
def _state_backward(Q, R, DQ, DY, H, DI, DH,
    ...
    for block in range(tl.cdiv(C, BC)):
        ...
        r, dr = tl.load(R+qo+raw_delta*(C*K), qm, 0), tl.load(DY+vo, vm, 0).to(tl.float32)
        dh = tl.dot(r, dr, dh, input_precision='tf32x3')
        q, dq = tl.load(Q+qo+raw_delta*(C*K), qm, 0), tl.load(DQ+vo, vm, 0)
...
class _FusedProjectedCoarse(torch.autograd.Function):
    ...
    def forward(ctx, projected, beta, k, gc, state, rn, qn, u, temperature, floor, output_dtype, raw_half, head_groups):
        ...
        tensors = tuple(x if raw_half and i in (1,2,3,5,6,7) else x.contiguous()
                        for i,x in enumerate((projected,beta,k,gc,state,rn,qn,u,temperature)))
        ...
        _finish_forward[(matrices,)](k, beta, gc, temperature, z2, numerator, h2,
            ...
            num_warps=4, enable_fp_fusion=False, RAW_HALF=raw_half, HEAD_GROUPS=head_groups)
    ...
    def backward(ctx, dy, ds, _dnorm, _dbound):
        ...
        dn, dzcoef, dkcoef, dbeta = (beta.new_empty(beta.shape) for _ in range(4))
        di = torch.empty((matrices,), device=projected.device, dtype=torch.float32)
        dgc, dt = gc.new_empty(gc.shape), torch.empty_like(ds)
        _scalar_backward[(matrices,)](beta, gc, temperature, stats, ds,
            dn, dzcoef, dkcoef, di, dbeta, dgc, dt, n, c, triton.next_power_of_2(c),
            floor*floor, shared_temp, num_warps=4, enable_fp_fusion=False, RAW_HALF=raw_half, HEAD_GROUPS=head_groups)
        dz, dq, du = (u.new_empty(u.shape) for _ in range(3))
        _activation_backward[(triton.cdiv(u.numel(), 256),)](
            projected, qread, u, dn, dzcoef, dz, dq, du, u.numel(), value_dim, 256,
            num_warps=4, RAW_HALF=raw_half, RAW_C=c)
        qg, rg, kg = (k.new_empty(k.shape) for _ in range(3))
        _query_backward[(matrices, triton.cdiv(c, 64), triton.cdiv(key_dim, 64))](
            dq, dy, state, k, dkcoef, qg, rg, kg,
            c, key_dim, value_dim, 64, 64, 16, num_warps=4, num_stages=1, RAW_HALF=raw_half)
...
With raw_half>0, six raw inputs retain the gap between complete Fenwick
period groups. States, projections, and returned gradients remain compact;
head_groups restores each original head's shared temperature and flags.
    """
    ...
    if head_groups < 1 or projected.shape[0] % head_groups:
        raise ValueError('Grouped coarse inputs must contain complete original heads.')
    if raw_half < 0 or (raw_half and projected.shape[1] != raw_half):
        raise ValueError('Grouped coarse inputs must contain complete active halves.')
    if not raw_half and head_groups != 1:
        raise ValueError('Head regrouping requires grouped raw inputs.')
